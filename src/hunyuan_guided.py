...
from pytorch3d.ops import interpolate_face_attributes, iterative_closest_point
...
import kaolin_sdf_ops as kaolin_sdf
...
def plot_in_grid_np(img_list, map_list, title_list, save_path="./pca_features.png"):
    """
    Plot two images in a grid and save to save_path.
    Assumes imgs are np arrays.
    """
    fig, axes = plt.subplots(2, len(img_list), figsize=(8, 4))

    for i, img in enumerate(img_list):
        img = img.transpose(1, 2, 0) # already in range [0, 1]
        axes[0, i].imshow(img)
        axes[0, i].axis('off')  # hide axis
        axes[0, i].set_title(title_list[i])
    
    for i, map1 in enumerate(map_list):
        axes[1, i].imshow(map1)
        axes[1, i].axis('off')
...
def viz_feats_pca(feats, save_path=None):
    """
    Visualize the features using PCA.
    Assumes the batch dim of the features is 1, so w/ shape (num_tokens, num_feats).
    """
    feats_np = feats.detach().cpu().numpy()
    feats_np = feats_np[1:, :]  # remove the CLS token --> (1369, 1536)

    pca = PCA(n_components=3)
    feats_pca = pca.fit_transform(feats_np) # (1369, 3)

    img_dim = int(feats_pca.shape[0] ** 0.5)
    feature_map = feats_pca.reshape(img_dim, img_dim, 3)
...
def latent2sdf(pred, xyz_samples, grid_size, vae, device, return_mesh=False, save_name='default'):
    ...
    if return_mesh:
        ...
        print(grid_logits.max(), grid_logits.min())
        return mesh_output
...
class Hunyuan3DDiTPipeline:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not os.path.exists(model_path):
            # try local path
            base_dir = './checkpoints/huggingface'
    ...
    def _export(self, latents, output_type, box_v, mc_level, num_chunks, octree_resolution, mc_algo, transform_cam_pose=None, return_sdf = False):
        if not output_type == "latent":
            ...
            if transform_cam_pose is not None:
                ...
                outputs = self.vae.latents2mesh( #latents2mesh_hamer(
                    latents,
                    bounds=box_v,
                    mc_level=mc_level,
                    num_chunks=num_chunks,
...
class Hunyuan3DDiTFlowMatchingPipeline_main(Hunyuan3DDiTPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        save_dir = f'./slurm/guidance_debug_log/19May/{exp_dir}'
        ...
        obj_alpha_ori = (np.array(image)[0][...,3] > 0).astype(np.uint8)
        ...
        J_regressor = torch.load('./hamer/J_regressor_hamer.pt')
        ...
        hunyuan_hoi_mesh = IO().load_mesh(hunyuan_hoi_mesh_path).to(device)
        hun_hoimesh_moge = transform_hunyuan2moge(hunyuan_hoi_mesh, T_h2m) 
        
        # combine hand and object masks TODO: can also just use the alpha channel of the bckg removed original input image
        moge_hoi_mask_path = moge_mesh_path.split('/mesh.ply')[0] + '/mask.png'
        moge_hoi_mask = cv2.imread(moge_hoi_mask_path, cv2.IMREAD_GRAYSCALE)
        moge_hoi_mask = torch.tensor(moge_hoi_mask, device=device).float().unsqueeze(0) # (1, h, w)
        moge_hoi_mask = (moge_hoi_mask > 0).bool() # convert to bool
        
        moge_verts, moge_faces = load_ply(moge_mesh_path)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            moge_sil = sil_renderer(moge_mesh)[..., 3] 
        ...
        for i, t in enumerate(tqdm(timesteps_obj, disable=not enable_pbar, desc="Diffusion Sampling:")):
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred_obj = noise_pred_obj_uncond + obj_guidance_scale_upt * (noise_pred_obj_cond - noise_pred_obj_uncond)
                ...
                with torch.enable_grad():
                    if i == handopt_start_step: # pre-guidance step: optimize hands while reconstructing the object separately
                        ...
                        for k in range(optimization_steps_hand):
                            ...
                            total_hand_loss = (
                                1e-2 * loss_2d_kps + # 1e-1 o iyi ornekte 1e-4
                                1 * loss_normal_hand +
                                10 * loss_disp_hand +
                                1 * loss_silhouette_hand +
                                1e-2 * loss_hand_trans 
                                # 1e-2 * loss_hand_rot_reg
                            )
                        ...
                        for k in range(optimization_steps_hand // 2): # optimization_steps_joint
                            ...
                            with torch.cuda.amp.autocast(enabled=False):
                                ...
                                # normal_map_white_bg = visualize_w_white_bg(renderer, transformed_obj_mesh)
                                # plti(normal_map_white_bg, save=True, prefix=f'{save_dir}/rendered_normal_obj_t{i}_opt{k}.png')
                                if debugging:
                                    if k % 10 == 0:
                                        plot_in_grid(rendered_obj_normal, moge_normal * obj_alpha_ori, save_path=f'{save_dir}/rendered_obj_normal_t{i}_opt{k}.png')
                            
                            loss_normal = normal_alignment_loss(rendered_obj_normal, moge_normal, valid_mask=moge_obj_mask) #obj_alpha_ori.squeeze(-1) #moge_obj_mask) # moge_hoi_mask
                            loss_disp = F.l1_loss(rendered_obj_disp, moge_disp * moge_obj_mask)
                            loss_silhouette = torch.nn.functional.binary_cross_entropy(rendered_obj_sil.float(), obj_alpha_ori.squeeze(-1).float())
                            # F.l1_loss(rendered_obj_sil, obj_alpha_ori.squeeze(-1).float()) #moge_sil * moge_obj_mask)
                            ...
                            total_obj_loss = (
                                1 * obj_loss +
                                10 * loss_normal + #10
                                10 * loss_disp + # 10, 1 o iyi ornekte
                                10 * loss_silhouette + #10 or 1?
                                1e-3 * obj_verts_loss + # 1e-2
                        ...
                        for k in range(optimization_steps_joint):
                            ...
                            # hand losses
                            loss_2d_kps = F.mse_loss(opt_2d_kps, torch.tensor(hamer_2d_kps, device=device).float()) 
                            loss_normal_hand = normal_alignment_loss(rendered_normal_hand, moge_normal, valid_mask=moge_hoi_mask)
                            loss_disp_hand = F.l1_loss(rendered_disp_hand, moge_disp * moge_hoi_mask)
                            loss_hand_trans = (trans_hand ** 2).mean() # regularization on hand translation to prevent exploding translation
                            hand_loss = (
                                1e-4 * loss_2d_kps +
                                10 * loss_normal_hand +
                            ...
                            dists_oh = dists_obj_to_hand.squeeze(0).squeeze(-1)
                            # proximity_loss = dists_ho.mean() + dists_oh.mean()
                            margin = 0.01 #0.02 #0.01
                            far_margin = 0.005 #0.01 #0.005
                            attract = torch.clamp(dists_ho - margin, min=0)
                            repel = torch.clamp(far_margin - dists_ho, min=0)
                            distance_loss = attract.mean() #+ repel.mean()
                            ...
                            # hoi losses
                            loss_normal_hoi = normal_alignment_loss(rendered_hoi_normal, moge_normal, valid_mask=moge_hoi_mask) # moge_hoi_mask
                            loss_normal_obj = normal_alignment_loss(rendered_obj_normal, moge_normal, valid_mask=obj_alpha_ori.squeeze(-1))#moge_obj_mask)
                            loss_disp_hoi = F.l1_loss(rendered_hoi_disp, moge_disp)
                            loss_disp_obj = F.l1_loss(rendered_obj_disp, moge_disp * moge_obj_mask)
                            loss_silhouette_hoi = torch.nn.functional.binary_cross_entropy(rendered_hoi_sil, moge_sil.float()) # F.l1_loss(rendered_hoi_sil, moge_sil)
                            loss_silhouette_obj = torch.nn.functional.binary_cross_entropy(rendered_obj_sil, obj_alpha_ori.squeeze(-1).float()) # F.l1_loss(rendered_obj_sil.to(torch.uint8), obj_alpha_ori.squeeze(-1).to(torch.uint8))
                            ...
                            total_loss = (
                                w_intersection * loss_intersection + # 1e-6
                                10 * distance_loss + # 1 distance, 1e-3 intersection o iyi ornekte
                                10 * loss_normal_hoi +
                                10 * loss_disp_hoi +
            ...
            if callback is not None and i % callback_steps == 0:
                step_idx = i // getattr(self.scheduler, "order", 1)
                callback(step_idx, t, hand_outputs)
                callback(step_idx, t, obj_outputs)
            ...
            with torch.no_grad():
                ...
                if i == num_inference_steps - 1 and debugging: # save the final meshes, also check the middle stp of guidance
